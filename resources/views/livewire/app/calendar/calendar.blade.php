<div class="md:container py-6 mx-auto grid my-6 text-gray-700  dark:text-gray-200 ">
    ...
    <div x-data="calendarComponent()"  id="calendarComponent"
        ...
        {{-- <x-modals.modal x-on:added="showModal = false" @keydown.escape.window="showModal = false" show="showModal" maxWidth="3xl">
            <x-slot name="title">
            ...
            <x-slot name="footer">
                    <x-danger-button x-on:click="showModal = false" >
                        {{ __('اغلاق') }}
                    </x-danger-button>
            </x-slot>
        </x-modals.modal> --}}

        <livewire:app.calendar.includes.add-event-modal :clinics="$clinics"></livewire:app.calendar.includes.add-event-modal>
        <div>
            {{-- <livewire:app.patient.includes.create-patient-modal :withButton="false" :clinics="['1' => 'العيادة الرئيسية']"></livewire:app.patient.includes.create-patient-modal> --}}
        </div>
...
@push('styles')
    ...
    <style>
        ...
        .fc-day-past .fc-daygrid-day-frame{
            /* background-color: #804bb84f !important; */
            color: ;
        }
    ...
    <script>
        function calendarComponent() {
            return {
                ...
                backgroundColors: ['#2196F3', '#009688', '#9C27B0', '#FFEB3B', '#afbbc9', '#4CAF50', '#2d3748', '#f56565', '#ed64a6'],
                ...
                init() {
                    ...
                    this.flatpickerupdateEndInstance = this.createFlatpickr('#update-evert-end-datepicker');
                        ...
                        openAddEventModal(info = null)
                        {
                            ...
                            window.dispatchEvent(new CustomEvent('update-all-day', { detail: info.allDay }));
                        ...
                        updateEventDate(info) {
                            @this.dispatch('updateEventDateAction', [info.event.id, info.event.startStr, info.event.endStr, info.event.allDay]);
                        },
                        updateEventAction()
                        {
                        ...
                        parseDates(dateString) { // Maybe it's too much but it works
                            ...
                            const parsedDateToStringWithAnotherFormat = this.flatpickrInstance.formatDate(parsedDateFromStringToDate, "Y-m-d H:i:s");
                            const parsedDateWithDesiredFormat = this.flatpickrInstance.parseDate(parsedDateToStringWithAnotherFormat);
