<div>
    ...
    @endif
    <div x-on:updated.window="{{ $showName }} = false">
        <x-modals.modal @keydown.escape.window="{{ $showName }} = false" show="{{ $showName }}" maxWidth="3xl">
            <x-slot name="title">
                تعديل الخدمة الطبية
            </x-slot>
            ...
            <x-slot name="content">
                ...
                <div class="flex flex-wrap">
                    <div class="mt-4 w-full  px-2">
                        ...
                        <x-input type="text" wire:model="name" withError="name"
                    ...
                    <div class="mt-4 w-full md:w-1/2 px-2">
                        <label for="price_update" class="font-bold">
                            السعر
                            <span class="text-red-600">*</span>
                        </label>
                        <x-input type="number" wire:model="price" withError="price"
                            class="mt-4 bg-gray-100 px-4 py-2 rounded  text-sm text-gray-500 w-full break-all" autofocus
                            autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" />
                    </div>
                    ...
                    <div x-data="colorSelectorUpdate('{{ $color }}')" class="inline-flex items-center w-full" x-cloak>
                        <div class="w-[90%]">
                            <label class="block font-bold px-3 py-4 ">لون
                                مميز</label>
                            <input x-ref="colorInputUpdate" type="text" placeholder="Pick a color"
                    ...
                    <div class="w-full px-2 mt-4 ">
                        <label for="description_update" class="font-bold">
                            الوصف
                        </label>
                        <textarea id="description_update" placeholder="اكتب هنا المعلومات الاضافية الخاصة بالخدمة ..." wire:model="description"
            ...
            <x-slot name="footer">
                <button class="mx-2 btn-dark text-sm " wire:click="updateClinicServiceAction"
...
@push('scripts')
    <script>
        ...
        function colorSelectorUpdate(initializedColor) {
            return {
                ...
                backgroundColorSelected: initializedColor,

                changeColor(selectedColor) {
                    this.backgroundColorSelected = selectedColor;
                    if (this.$refs.colorButton) {
                        this.$refs.colorButton.style.backgroundColor = selectedColor;
                    }
                    if (this.$refs.colorInputUpdate) {
                        this.$refs.colorInputUpdate.value = selectedColor;
                        triggerLivewireInputEvent(this.$refs.colorInputUpdate);
                    }
                },
            }
        }
