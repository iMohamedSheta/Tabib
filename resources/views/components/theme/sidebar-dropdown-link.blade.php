...
<li class="relative" x-data="dataSidebarDropDownMenu()"  >
    @if(in_array(Route::currentRouteName(), $activeRoutes))
    <!-- Active items have the snippet below -->
    <span
    class="absolute inset-y-0 right-0 w-1 bg-gray-300 dark:bg-purple-600 rounded-tr-lg rounded-br-lg"
    aria-hidden="true"
    ></span>
    @endif
    ...
    <button
        ...
        <svg class="w-4 h-4 transation duration-150" :class="isPagesMenuOpen && 'rotate-180'" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd"
                d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z"
                clip-rule="evenodd"></path>
        </svg>
    </button>
    <div x-show="isPagesMenuOpen" style="display: none" class="mr-8 ml-4" x-transition:enter="transition-all ease-in-out duration-300"
    ...
    x-transition:leave-end="opacity-0 transform -translate-y-2 max-h-0">
        <ul
            class="mb-2  overflow-hidden text-sm bg-purple-800 font-medium text-gray-500 rounded-md shadow-inner  dark:text-gray-400 dark:bg-c-gray-900"
            aria-label="submenu">
            {{ $slot }}
        </ul>
    </div>
