...
<li
    class="relative flex-1 after:content-['']  after:w-0.5 after:h-full  {{ $active || $nextIsActive ? 'after:bg-purple-700' : 'after:bg-gray-200' }} after:inline-block after:absolute after:-bottom-12 after:left-1/2 shadow-lg">
    <div class="flex items-center justify-center gap-8 w-full max-w-sm">
        <div
            class="flex items-center gap-3.5 {{ $active ? 'bg-purple-50 border-purple-600' : 'bg-gray-50 border-gray-50' }}  p-3.5 rounded-xl relative z-10 border  w-full">
            @if ($done)
